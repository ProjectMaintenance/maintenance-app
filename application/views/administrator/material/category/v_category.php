    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title">
                    <a href="<?= site_url('administrator/add_category') ?>" class="btn btn-primary"><i
                    ...
                    <div class="modal fade" id="upload_excel_category" data-backdrop="static" data-keyboard="false"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <?= form_open_multipart('administrator/upload_category', array('id' => 'form_upload_excel_category')) ?>
...
<div class="modal fade" id="updatecategory<?= $value->id_category; ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <?= form_open('administrator/update_category', array('id' => 'form-update-category' . $value->id_category)); ?>
            <div class="card-body">
                <div class="form-group">
...
$(document).ready(function() {
    ...
    $('#form-update-category<?= $value->id_category ?>').validate({
        rules: {
            code_category: {
                required: true,
                remote: {
                    url: "<?= site_url('administrator/check_code_category') ?>",
                    type: "POST",
                    data: {
    ...
    $(document).on('click', 'button[data-id-category]', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '<?= site_url('administrator/delete_category') ?>',
                    data: {
    ...
    $('#form_upload_excel_category').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('administrator/upload_category') ?>',
            data: formData,
            dataType: 'JSON',
            contentType: false,
            processData: false,
                ...
                Swal.fire({
                    ...
                    if (response.success) {
                        window.location.href =
                            "<?= site_url('administrator/category') ?>";
                    }
                });
            },
            error: function(jqXHR, textStatus, errorThrown) {
